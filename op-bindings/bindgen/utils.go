...
func readJSONFile(filePath string, result interface{}) error {
	log.Debug("Reading contract list", "filePath", filePath)
...
func mkTempArtifactsDir() (string, error) {
	...
	log.Debug("Created temporary artifacts directory", "dir", dir)
	return dir, nil
}
...
func writeContractArtifacts(tempDirPath, contractName string, abi, bytecode []byte) (string, string, error) {
	log.Debug("Writing ABI and bytecode to temporary artifacts directory", "contractName", contractName, "tempDirPath", tempDirPath)
...
func genContractBindings(abiFilePath, bytecodeFilePath, goPackageName, contractName string) error {
	...
	log.Debug("Generating contract bindings", "contractName", contractName, "outFilePath", outFilePath)
