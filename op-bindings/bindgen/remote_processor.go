...
func (gen *remoteBindingsGenerator) writeContractMetadata(contractMetaData contractMetadata, contractName string, fileTemplate *template.Template) error {
	...
	log.Debug("Successfully wrote contract metadata", "contractName", contractName, "metadataFilePath", metadataFilePath)
	return nil
}
...
func (gen *remoteBindingsGenerator) fetchContractData(contract *remoteContract) error {
	...
	if gen.compareDeploymentBytecode || gen.compareInitBytecode {
		...
		if !ok {
			log.Warn(fmt.Sprintf("No deployment address was found for %s for chain ID: %d, skipping bytecode comparisons", contract.Name, gen.compareChainId))
...
func (gen *remoteBindingsGenerator) processContracts(contracts []remoteContract) error {
	for _, contract := range contracts {
		log.Info("Generating bindings and metadata for remote contract", "contractName", contract.Name)

		if err := gen.fetchContractData(&contract); err != nil {
			return fmt.Errorf("error fetching contract data for %s: %w", contract.Name, err)
		}

		abiFilePath, bytecodeFilePath, err := writeContractArtifacts(gen.tempArtifactsDir, contract.Name, []byte(contract.Abi), []byte(contract.InitBytecode))
		if err != nil {
			return err
		}

		err = genContractBindings(abiFilePath, bytecodeFilePath, gen.bindingsPackageName, contract.Name)
		if err != nil {
			return err
		}
		...
		if contract.HasImmutables && contract.Create2ProxyDeployed {
			contractMetadataWithImmutablesFileTemplate := template.Must(template.New("contractMetadataWithImmutables").Parse(contractMetadataWithImmutablesTemplate))
