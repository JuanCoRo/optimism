...
func (client *Client) fetchEtherscanApi(url string) (apiResponse, error) {
	for retries := 0; retries < client.apiMaxRetries; retries++ {
		...
		if response.Message == "NOTOK" && response.Result == "Max rate limit reached" {
			log.Warn(fmt.Sprintf("Reached API rate limit, waiting %v and trying again", client.apiRetryDelay))
			time.Sleep(client.apiRetryDelay)
			continue
		}
	...
	return apiResponse{}, fmt.Errorf("failed to fetch from Etherscan API after %d retries", client.apiMaxRetries)
}
...
func (client *Client) fetchEtherscanRpc(url string) (rpcResponse, error) {
	for retries := 0; retries < client.apiMaxRetries; retries++ {
		...
		err = json.Unmarshal(body, &response)
		if err != nil {
			log.Warn(fmt.Sprintf("Failed to unmarshal response as rpcResponse, waiting %v and trying again", client.apiRetryDelay))
			time.Sleep(client.apiRetryDelay)
			continue
		}
	...
	return rpcResponse{}, fmt.Errorf("failed to fetch from Etherscan RPC after %d retries", client.apiMaxRetries)
}
