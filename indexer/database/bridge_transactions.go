...
func (db *bridgeTransactionsDB) L1LatestFinalizedBlockHeader() (*L1BlockHeader, error) {
	// A Proven or Finalized Event
	l1ProvenQuery := db.gorm.Table("l2_transaction_withdrawals").Order("timestamp DESC").Limit(1)
	...
	l1ProvenQuery = l1ProvenQuery.Order("l1_contract_events.timestamp DESC").Select("l1_contract_events.*")

	l1FinalizedQuery := db.gorm.Table("l2_transaction_withdrawals").Order("timestamp DESC").Limit(1)
	...
	l1FinalizedQuery = l1FinalizedQuery.Select("l1_contract_events.*")

	l1Query := db.gorm.Table("((?) UNION (?)) AS finalized_bridge_events", l1ProvenQuery, l1FinalizedQuery)
...
func (db *bridgeTransactionsDB) L2LatestFinalizedBlockHeader() (*L2BlockHeader, error) {
	// Latest Relayed CrossDomain Message. Unfortunately, since we dont have any low-level indicator of included
	// L1 deposits on L2, we can only rely on the L2CrossDomainMessenger and its relayed message marker. For
	// low-activity chains, this can result in an overlap in re-visited blocks between restarts.
	//   - If this becomes a problem, we may introduce stateful markers for our processors to read from on startup
	l2Query := db.gorm.Table("l2_bridge_messages").Order("timestamp DESC")
	l2Query = l2Query.Joins("INNER JOIN l2_contract_events ON l2_contract_events.guid = l2_bridge_messages.relayed_message_event_guid")
